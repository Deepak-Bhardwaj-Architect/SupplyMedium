...
function  showDepartmentfeed()
{
  ...
  // alert(departmentKey);
    $.ajax({
        ...
        {
            //alert("data"+data);            
            document.getElementById('annomt_list').innerHTML = data;
            splitFeedKeys();
        },
    ...
    $.ajax({
		...
		success : function(data)
		{
                    ...
                    feedContent +="<div class='dilbag-lca'>";
                        ...
                        feedContent +="</div>";
                    feedContent +="</div>";
                    feedContent +="<div style='background:#F8F8F8;float:left;width:610px;display:none;padding:15px;' id='cmnt_"+id+"'>";
                        feedContent +="<textarea id='cmntddd_"+id+"' placeholder='Write comment' style='width:560px;height:50px;background:#FFFFFF;'></textarea>";
                    ...
                    feedContent +="<ul id='all_comment_"+id+"' style='background:#F8F8F8;float:left;width:560px;padding-top:15px;'></ul></div>";
...
function select_feed_comment(id)
{
    $.ajax({
		...
		data : {
			'ids' : id,
                        'selectInternalFeedComment':'selectInternalFeedComment'
		},		
		success : function(data)
		{
                    //alert(data);
