...
import supply.medium.home.edi.GenerateEDI;
...
public class GenerateINV {
    ...
    public static void generate(String path, String fileName, String qteKey, String fromcompanyKey, String tocompanyKey,String sendEmail) {
        try {
...
//            Date date = null;
            ...
            TransactionInvBean tb = null;
            tb = TransactionInvMaster.showByTransactionKeyFromToCompany(qteKey, fromcompanyKey, "from");

            toOutsider = tb.getInv_is_outside();
            ...
            transId = tb.getInv_trans_rqf_key();
            transDate = tb.getInv_created_on();

            String aD=transDate.split(" ")[0];
            if(aD.contains("-"))
            transDate=aD.split("-")[2]+"-"+aD.split("-")[1]+"-"+aD.split("-")[0];
            else if(aD.contains("/"))
            transDate=aD.split("/")[2]+"-"+aD.split("/")[1]+"-"+aD.split("/")[0];
            else if(aD.contains("."))
            transDate=aD.split(".")[2]+"-"+aD.split(".")[1]+"-"+aD.split(".")[0];
            ...
            if (toOutsider.equals("yes")) {
                String outsiderDetail[] = tb.getInv_is_outside_address().split("@#@#@");
                ...
                tb = TransactionInvMaster.showByTransactionKeyFromToCompany(qteKey, tocompanyKey, "to");
            ...
            toState = StateMaster.showStateFromKey(toState);
            fromState = StateMaster.showStateFromKey(fromState);
            ...
            Paragraph heading = new Paragraph("Invoice by " + fromName.toUpperCase() + "\n\n", fontHeading);
            ...
            ArrayList itemList = TransactionInvItemMaster.showInvItemFromInvKey(qteKey);
            ...
            for (int i = 0; i < itemList.size(); i++) {
                trib = (TransactionInvItemBean) itemList.get(i);

                //date = df.parse(trib.getShipDate());
                aD=trib.getShipDate().split(" ")[0];
                if(aD.indexOf("-")!=-1)
                transDate=aD.split("-")[2]+"-"+aD.split("-")[1]+"-"+aD.split("-")[0];
                if(aD.indexOf("/")!=-1)
                transDate=aD.split("/")[2]+"-"+aD.split("/")[1]+"-"+aD.split("/")[0];
                ...
                cell = new PdfPCell(new Paragraph(trib.getQuantityShipped()+ " " + QuantityTypeMaster.showCodeByKey(trib.getQuantityShippedUnitKey()), fontDataNormal));
                cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
                cell.setBorderWidth((float) 1.0);
                cell.setPadding(2.0f);
                table.addCell(cell);

                cell = new PdfPCell(new Paragraph(transDate, fontDataNormal));
                cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
                cell.setBorderWidth((float) 1.0);
                cell.setPadding(2.0f);
                table.addCell(cell);
                ...
                a = Double.parseDouble(trib.getQuantityShipped());
            ...
            tb = TransactionInvMaster.showByKey(qteKey);
            ...
            cell.setPadding(2.0f);
            table.addCell(cell);
            total = Double.parseDouble(tb.getInv_total_amount());
            ...
            table.addCell(cell);
            table.addCell(cell);
            per = Double.parseDouble(tb.getInv_tax_percentage());
            ...
            cell.setPadding(2.0f);
            table.addCell(cell);
            bill = Double.parseDouble(tb.getInv_total_billing_amount());
            ...
            if (tb.getBill_of_landing() != null && !tb.getBill_of_landing().equals("")) {
                Paragraph bill_of_landing = new Paragraph("Bill of Landing Information", fontDataBold);
                bill_of_landing.setAlignment(Element.ALIGN_LEFT);
                bill_of_landing.setSpacingAfter(5);
                document.add(bill_of_landing);
                
                String value="Bill of Landing No. : "+tb.getBill_of_landing()
                        + "\nFreight Weight : "+tb.getFreight_weight()+" "+QuantityTypeMaster.showCodeByKey(tb.getFreight_weight_unit())
                        + "\nDate Shipped : "+tb.getShipped_date();
                

                Paragraph bill_of_landing2 = new Paragraph(value, fontDataNormal);
                bill_of_landing2.setAlignment(Element.ALIGN_LEFT);
                document.add(bill_of_landing2);
            }
            
            TermsConditionsBean tcb = TermsConditionsMaster.show(fromcompanyKey, "INV");
            if (tcb != null) {
            ...
            document.close();
            GenerateEDI.generateINV(transId, path, fileName, fromcompanyKey, tocompanyKey);
            if(sendEmail.equals("Yes"))
            {
                HTMLMailComposer mcomp = new HTMLMailComposer();
                String message = mcomp.composeINV(fromName + " (" + fromEmail + ") has sent you invoice.");
                AttachmentMailer am = new AttachmentMailer();
                String to[] = {toEmail};
                am.composeAndSendMail(to, fromName + " has sent you invoice via SupplyMedium", message, filePath1, fileName + ".pdf");
            }
        } catch (Exception ex) {
            ErrorMaster.insert("Exception at generate in GenerateINV : " + ex.getMessage());
        }
