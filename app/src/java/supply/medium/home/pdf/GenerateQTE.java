...
public class GenerateQTE {
    ...
    public static void generate(String path, String fileName, String qteKey, String fromcompanyKey, String tocompanyKey) {
        try {
            ...
            TransactionQteBean tb = null;
            tb = TransactionQteMaster.showByTransactionKeyFromToCompany(qteKey, fromcompanyKey, "from");

            toOutsider = tb.getQ_is_outside();
            ...
            transId = tb.getQ_trans_rqf_key();
            transDate = tb.getQ_created_on();

            date = df.parse(transDate);

            if (toOutsider.equals("yes")) {
                String outsiderDetail[] = tb.getQ_is_outside_address().split("@#@#@");
                ...
                tb = TransactionQteMaster.showByTransactionKeyFromToCompany(qteKey, tocompanyKey, "to");
            ...
            Paragraph heading = new Paragraph("Quote by " + fromName.toUpperCase() + "\n\n", fontHeading);
            ...
            ArrayList itemList = TransactionQteItemMaster.showQteItemFromQteKey(qteKey);
            ...
            for (int i = 0; i < itemList.size(); i++) {
                trib = (TransactionQteItemBean) itemList.get(i);
...
//                date = df2.parse(trib.getShipDate());
            ...
            tb = TransactionQteMaster.showByKey(qteKey);
            ...
            cell.setPadding(2.0f);
            table.addCell(cell);
            total = Double.parseDouble(tb.getQ_total_amount());
            ...
            table.addCell(cell);
            table.addCell(cell);
            per = Double.parseDouble(tb.getQ_tax_percentage());
            ...
            cell.setPadding(2.0f);
            table.addCell(cell);
            bill = Double.parseDouble(tb.getQ_total_billing_amount());
            ...
            TermsConditionsBean tcb = TermsConditionsMaster.show(fromcompanyKey, "QTE");
            if (tcb != null) {
            ...
            document.close();
            GenerateEDI.generateQTE(transId, path, fileName, fromcompanyKey, tocompanyKey);
            HTMLMailComposer mcomp = new HTMLMailComposer();
            String message = mcomp.composeQTE(fromName + " (" + fromEmail + ") has sent you quote.");
            AttachmentMailer am = new AttachmentMailer();
            String to[] = {toEmail};
            am.composeAndSendMail(to, fromName + " has sent you quote via SupplyMedium", message, filePath1, fileName + ".pdf");
        } catch (Exception ex) {
            ErrorMaster.insert("Exception at generate in GenerateQTE : " + ex.getMessage());
        }
