...
<%
    ...
    String folderurl = SmProperties.dataPathUrl + "company-" + companyKey + "/department feed";
    folderPath=folderPath.replace(File.separator+"app"+File.separator, File.separator+"zData"+File.separator);
    folderurl=folderurl.replace(File.separator+"app"+File.separator, File.separator+"zData"+File.separator);
    File folder = null;
    File[] files = null;
    int folderSize = 0;
    ...
    for (int i = 0; i < groupList.size(); i++) {
        ...
        folder = new File(folderPath + File.separator + ub.getFolderName()+File.separator);
        
        if (folder.exists()) {
            
            folderSize = new File(folderPath + File.separator + ub.getFolderName()+File.separator).listFiles().length;
        }
%>
