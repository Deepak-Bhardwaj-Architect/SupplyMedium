...
function get_notification_counting()
{  
  $.ajax({
        type: "POST",
        dataType: 'text',
        url: getBaseURL() + "/dilbag",
        data:
                {
        ...
        {
            //alert(data);            
            var notificationCount = parseInt(data);
            if( notificationCount === 0  || notificationCount === "NaN" )
	{
		$("#notification_count").hide();
...
function parseLatestNotification( notificationArr )
{
	...
	for( var i=notificationArr.length-1;i>=0;i-- )
	{
		...
		connStr += "<div class='user_container' id='"+id+"'  onclick=\"javascript:$('#"+id+"').fadeOut();\">";
