...
<div class="Cus_Popup_Outline quote_popup_outline">
    <div class="Popup_Tilte_NewGroup Gen_Popup_Title" style="border-radius: 0px;">
        <div style="padding: 0px 0px 0px 15px; float: left" id="quote_tl">QTE<%=request.getParameter("quoteKey")%></div>
<%
                            TransactionQteBean objTr=TransactionQteMaster.showByKey(request.getParameter("quoteKey"));
    String nme=null;
    String cnt=null;
    String stt=null;
    String cty=null;
    ...
    if(objTr.getQ_is_outside().equalsIgnoreCase("yes"))
    {
        String outsider[]=objTr.getQ_is_outside_address().split("@#@#@");
        nme=outsider[0];
...
%>
        <div style="float: right;width:100px;" onclick="window.location.href='';">
            <a onClick="this.href = '<%=request.getRealPath("") + File.separator + "cropData/company-"+session.getAttribute("companyKey")+"/transaction/QTE"+request.getParameter("quoteKey")+".pdf" %>'" target="_blank">
                <img src="inside/open.png" style="margin-top:10px;">
            </a>
            <div class="Popup_Close_NewGroup Gen_Cus_Popup_Close" onclick="$('#quote_popup').fadeOut();">
            </div>
        </div>
    </div>
    <div id="quote_form_popup_content" style="margin:0px;float:left;height:auto;" class="quote_form_content">
        <form method="post" action="TransQuoteAction">
            <div class="quote_popup_form mCustomScrollbar _mCS_2" id="quote_popup_form" style="margin-top:10px;overflow:auto;float:left;height:700px;width:99%;">
                <div class="mCustomScrollBox mCS-dark-thick" id="mCSB_2" style="position:relative; height:100%; overflow:hidden; max-width:100%;">
                    <div class="mCSB_container mCS_no_scrollbar" style="position: relative; top: 0px;">
                        <input type="hidden" id="quoteKey" name="quoteKey" value="<%=request.getParameter("quoteKey")%>">
                        <input type="hidden" id="quoteAction" name="quoteAction">
                        ...
                        <div class="quote_supplier_det"  style="width:440px;">
                            <div class="sub_heading">Buyer Details</div>
                            <div class="trans_row">
                                <div class="checkContainer">
                                    <input type="checkbox" <% if(objTr.getQ_is_outside().equalsIgnoreCase("yes")){out.println("checked");}%> disabled class="checkbox" id="popup_outside_supplier">
                                    <div>
                                    </div>
                                </div>
                            ...
                            <div class="popup_outside_sup_email_content" style="width: 480px; <% if(objTr.getQ_is_outside().equalsIgnoreCase("no")){out.println("display: none;");}%>">
                        ...
                        <div class="items" id="popup_items">         
                                <%
                                    ArrayList itemList = TransactionQteItemMaster.showQteItemFromQteKey(request.getParameter("quoteKey"));
                            ...
                            <div id="item1" class="item" style="width:970px;float:left;margin-top:5px;margin-left:15px;margin-right:0px;">
                                ...
                                <input type="text" autocomplete="off" class="textbox" maxlength="80" id="part_no1" style="width:48px;margin-right:10px;" disabled="" value="<%=transactionQteItemBean.getPartNo()%>">
                                ...
                                <input type="text" autocomplete="off" class="textbox" maxlength="80" id="brcd_no1" style="width:48px;margin-right:10px;" disabled="" value="<%=transactionQteItemBean.getBarcode()%>">
                                <input type="text" autocomplete="off" class="textbox" maxlength="12" id="quantity1" style="width:28px;" disabled="" value="<%=transactionQteItemBean.getQuantity()%>">
                                ...
                                <input type="text" autocomplete="off" class="textbox" id="ship_date1" style="width:74px;margin-right:10px;" disabled="" value="<%=transactionQteItemBean.getShipDate().split(" ")[0]%>">
                        ...
                        <div class="price_det" id="price_det">
                            ...
                            <% TransactionQteBean qtb=TransactionQteMaster.showByKey(request.getParameter("quoteKey"));
                            %>
                            <div class="price_det_content">
                                ...
                                <label><%=qtb.getQ_tax_percentage() %>%</label>
                            </div>
                            <div class="price_det_content">
                                <label class="price_det_lbl" id="tot_price_lbl"> Total Price: </label>
                                <label class="price_det_lbl" id="popup_tot_price_amt"> <%=qtb.getQ_total_billing_amount() %> </label>
                            </div>
                        </div>
                        <%
                            String display = "none";
                            String name = "";
                            String result = "";
                            ArrayList inquiryMessageList = InquiryMaster.showAllInquiryByTypeAndKey("QTE", request.getParameter("quoteKey"));
                            InquiryBean ib = null;
                        ...
                        <div class="new_inquire" id="rfq_popup_new_inquire"  style="display:none">
                            <div class="inquire_row">
                                ...
                                <input type="submit" class="gen-btn-blue" style="margin-left:30px;margin-top:7px;" id="quote_inquire_send" value="Send" onclick="$('#quoteAction').val('Inquired');
                                        return validateTransactionInquireMessage();">
                                <label class="quote_error" style="width:300px;margin-left: 125px;margin-top: 10px;" id="inquire_error">
                                </label>
                            </div>
                        </div>    
                        <div id="supplier_ctrls" class="supplier_controls">
                            <%
                                if (!session.getAttribute("userKey").toString().equals(request.getParameter("supplierKey")) && (objTr.getQ_trans_status().equals("QTE Sent") || objTr.getQ_trans_status().equals("Inquired"))){  %>
                            <input type="submit" class="gen-btn-blue" style="margin-left:30px;" onclick="$('#quoteAction').val('Accepted');" id="quote_accept_btn" value="Accept">
                            <input type="submit" class="gen-btn-red" style="margin-left:30px;" onclick="$('#quoteAction').val('Rejected');" id="quote_reject_btn" value="Reject">
                            <% }%>
                            <input type="button" class="gen-btn-blue" style="margin-left:30px;" id="supp_quote_inquire_btn" value="OK" onclick="$('#quote_popup').fadeOut();">
                            <%
                            if(!qtb.getQ_trans_status().equalsIgnoreCase("Accepted"))
                            {
                            %>
                            <input type="button" class="gen-btn-blue" style="margin-left:30px;width:130px;" id="buy_quote_inquire_btn" value="Send Message" onclick="transactionEnquireBox();">
                            <%
                            }
                            %>
                        </div>
