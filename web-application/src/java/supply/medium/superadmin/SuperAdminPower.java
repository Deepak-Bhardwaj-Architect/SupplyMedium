    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("typ").equals("cmpny_mr_dtl")) {
                ...
                int usr = 0,rfq_inside=0,qte_inside=0,po_inside=0,inv_inside=0,rfq_outside=0,qte_outside=0,po_outside=0,inv_outside=0,rfq_inside_items=0,qte_inside_items=0,po_inside_items=0,inv_inside_items=0,rfq_outside_items=0,qte_outside_items=0,po_outside_items=0,inv_outside_items=0,no_of_payments=0;
                ...
                db_qrs.close();
                ResultSet rfq_insides = db_qrs.select("SELECT count(*) from trans_rfq_master where (company_key_from=" + request.getParameter("nmbr") + " and company_key_to!='null') or (company_key_to=" + request.getParameter("nmbr") + " and company_key_from!='null');");
                if (rfq_insides.next()) {
                    rfq_inside= rfq_insides.getInt(1);
                }
                db_qrs.close();
                ResultSet rfq_outsides = db_qrs.select("SELECT count(*) from trans_rfq_master where (company_key_from=" + request.getParameter("nmbr") + " and company_key_to='null') or (company_key_to=" + request.getParameter("nmbr") + " and company_key_from='null');");
                if (rfq_outsides.next()) {
                    rfq_outside= rfq_outsides.getInt(1);
                }
                db_qrs.close();
                ResultSet qte_insides = db_qrs.select("SELECT count(*) from transaction_master where (company_key_from=" + request.getParameter("nmbr") + " and company_key_to!='null') or (company_key_to=" + request.getParameter("nmbr") + " and company_key_from!='null');");
                if (qte_insides.next()) {
                    qte_inside= qte_insides.getInt(1);
                }
                db_qrs.close();
                ResultSet qte_outsides = db_qrs.select("SELECT count(*) from transaction_master where (company_key_from=" + request.getParameter("nmbr") + " and company_key_to='null') or (company_key_to=" + request.getParameter("nmbr") + " and company_key_from='null');");
                if (qte_outsides.next()) {
                    qte_outside= qte_outsides.getInt(1);
                }
                db_qrs.close();
                ResultSet inv_insides = db_qrs.select("SELECT count(*) from transaction_master where (company_key_from=" + request.getParameter("nmbr") + " and company_key_to!='null') or (company_key_to=" + request.getParameter("nmbr") + " and company_key_from!='null');");
                if (inv_insides.next()) {
                    inv_inside= inv_insides.getInt(1);
                }
                db_qrs.close();
                ResultSet inv_outsides = db_qrs.select("SELECT count(*) from transaction_master where (company_key_from=" + request.getParameter("nmbr") + " and company_key_to='null') or (company_key_to=" + request.getParameter("nmbr") + " and company_key_from='null');");
                if (inv_outsides.next()) {
                    inv_outside= inv_outsides.getInt(1);
                }
                db_qrs.close();
                ResultSet po_insides = db_qrs.select("SELECT count(*) from transaction_master where (company_key_from=" + request.getParameter("nmbr") + " and company_key_to!='null') or (company_key_to=" + request.getParameter("nmbr") + " and company_key_from!='null');");
                if (po_insides.next()) {
                    po_inside= po_insides.getInt(1);
                }
                db_qrs.close();
                ResultSet po_outsides = db_qrs.select("SELECT count(*) from transaction_master where (company_key_from=" + request.getParameter("nmbr") + " and company_key_to='null') or (company_key_to=" + request.getParameter("nmbr") + " and company_key_from='null');");
                if (po_outsides.next()) {
                ...
                if (rs14.next()) {
                    qte_inside_items= rs14.getInt(1);
                }
                db_qrs.close();
                ...
                if (rs15.next()) {
                    qte_outside_items= rs15.getInt(1);
                }
                db_qrs.close();
...
//                if (rs16.next()) {
//                    qte_inside_items= rs16.getInt(1);
//                }
...
//                if (rs17.next()) {
//                    qte_outside_items= rs17.getInt(1);
//                }
//                db_qrs.close();
                ...
                if (rs20.next()) {
                    inv_inside_items= rs20.getInt(1);
                }
                ...
                if (rs21.next()) {
                    inv_outside_items= rs21.getInt(1);
                }
                db_qrs.close();
                ...
                out.print("<table class='admin_companies_trans'>\n"
                        ...
                        + "  </tr>\n"
                        + "  <tr style='border: none;'>\n"
                        + "<td style='border-right:black 1px solid;border-top:black 1px solid;'>"+qte_inside +"("+qte_inside_items +")</td>\n"
                        + "<td style='border-top:black 1px solid;'>"+qte_outside +"("+qte_outside_items +")</td>\n"
                        + "  </tr>\n"
                        + " </table>\n"
                        + "</td>\n"
                        + "<td style='padding:0px;'>\n"
                        ...
                        + "  </tr>\n"
                        + "  <tr style='border: none;'>\n"
                        + "<td style='border-right:black 1px solid;border-top:black 1px solid;'>"+inv_inside +"("+inv_inside_items +")</td>\n"
                        + "<td style='border-top:black 1px solid;'>"+inv_outside +"("+inv_outside_items +")</td>\n"
                        + "  </tr>\n"
                        + " </table>\n"
                        + "</td>\n"
                        + "<td style='padding:0px;'>\n"
