...
//                                + "<input style='margin-top:-5px;margin-left:20px;' type='submit' value='Save' class='gen-btn-Orange'>"
                            + "</div>"
                            + "<div style='width:500px;float:left;height: 287px;overflow-y: scroll;'><b>Product Images</b><br/>";
            if(pb.getPicsCount().equals(""))
            {
            ...
            else
            {
                    ...
                    result += "<img src='"+request.getRealPath("") + File.separator +"cropData/products/"+pics[i].replace("@#@","")+"' style='margin:10px;' title='"+pb.getItemName()+"' height='120' />";
            }
                     result += "</div>"
                        + "</form>"
                    + "</div>";
