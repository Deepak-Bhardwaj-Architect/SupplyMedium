    ...
    public static int checkLoginCredential(String email,String password,HttpSession session)
    {
        ...
        try
        {
            ...
            if(rs.next())
            {
                ...
                session.setAttribute("companyType", CompanyMaster.getCompanyTypeFromKey(session.getAttribute("companyKey").toString()));
                AccountPolicyBean apb=(AccountPolicyBean)AccountPolicyMaster.showAccountPolicy(session.getAttribute("companyKey").toString()).get(0);
                session.setMaxInactiveInterval(60*(Integer.parseInt(apb.getSessionEndTime())));
                result=1;
            }
