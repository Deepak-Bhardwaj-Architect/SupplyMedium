...
String q_total_billing_amount,String q_is_po_created) 
    ...
    public static int updatePoGenerated(String transKey,String status)
    {
        int result = 0;
        Connection con = null;
        Statement st = null;
        try {
            ...
            String query = "update transaction_master "
                    + "set q_is_po_created='"+status+"'"
                    + "where trans_key='"+transKey+"'";
            result = st.executeUpdate(query);
        } catch (Exception ex) {
            ErrorMaster.insert("Exception at updatePoGenerated in TransactionQteMaster : " + ex.getMessage());
        } finally {
            try {
                st.close();
                con.close();
