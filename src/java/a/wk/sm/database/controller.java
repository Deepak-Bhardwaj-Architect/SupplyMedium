    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("typ").equals("cmpny_mr_dtl")) {
                ...
                db_qrs.close();
                ResultSet rs4 = db_qrs.select("SELECT count(trans_id) FROM rfq where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null');");
                if (rs4.next()) {
                    rfq_inside= rs4.getInt(1);
                }
                db_qrs.close();
                ResultSet rs5 = db_qrs.select("SELECT count(trans_id) FROM rfq where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null');");
                if (rs5.next()) {
                    rfq_outside= rs5.getInt(1);
                }
                db_qrs.close();
                ResultSet rs6 = db_qrs.select("SELECT count(trans_id) FROM quote where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null');");
                if (rs6.next()) {
                    qte__inside= rs6.getInt(1);
                }
                db_qrs.close();
                ResultSet rs7 = db_qrs.select("SELECT count(trans_id) FROM quote where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null');");
                if (rs7.next()) {
                    qte__outside= rs7.getInt(1);
                }
                db_qrs.close();
                ResultSet rs8 = db_qrs.select("SELECT count(trans_id) FROM invoice where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null');");
                if (rs8.next()) {
                    inv__inside= rs8.getInt(1);
                }
                db_qrs.close();
                ResultSet rs9 = db_qrs.select("SELECT count(trans_id) FROM invoice where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null');");
                if (rs9.next()) {
                    inv__outside= rs9.getInt(1);
                }
                db_qrs.close();
                ResultSet rs10 = db_qrs.select("SELECT count(trans_id) FROM po where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null');");
                if (rs10.next()) {
                    po_inside= rs10.getInt(1);
                }
                db_qrs.close();
                ResultSet rs11 = db_qrs.select("SELECT count(trans_id) FROM po where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null');");
                if (rs11.next()) {
                    po_outside= rs11.getInt(1);
                }
                db_qrs.close();
                ResultSet rs12 = db_qrs.select("SELECT count(rfq_item_id)  FROM rfq_item where trans_id in (SELECT trans_id FROM rfq where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null'));");
                if (rs12.next()) {
                    rfq_inside_items= rs12.getInt(1);
                }
                db_qrs.close();
                ResultSet rs13 = db_qrs.select("SELECT count(rfq_item_id)  FROM rfq_item where trans_id in (SELECT trans_id FROM rfq where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null'));");
                if (rs13.next()) {
                    rfq_outside_items= rs13.getInt(1);
                }
                db_qrs.close();
                ResultSet rs14 = db_qrs.select("SELECT count(quote_item_id)  FROM quote_item where trans_id in (SELECT trans_id FROM quote where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null'));");
                if (rs14.next()) {
                    qte__inside_items= rs14.getInt(1);
                }
                db_qrs.close();
                ResultSet rs15 = db_qrs.select("SELECT count(quote_item_id)  FROM quote_item where trans_id in (SELECT trans_id FROM quote where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null'));");
                if (rs15.next()) {
                    qte__outside_items= rs15.getInt(1);
                }
                db_qrs.close();
                ResultSet rs16 = db_qrs.select("SELECT count(quote_item_id)  FROM quote_item where trans_id in (SELECT trans_id FROM quote where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null'));");
                if (rs16.next()) {
                    qte__inside_items= rs16.getInt(1);
                }
                ResultSet rs17 = db_qrs.select("SELECT count(quote_item_id)  FROM quote_item where trans_id in (SELECT trans_id FROM quote where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null'));");
                if (rs17.next()) {
                    qte__outside_items= rs17.getInt(1);
                }
                db_qrs.close();
                ResultSet rs18 = db_qrs.select("SELECT count(po_item_id)  FROM po_item where trans_id in (SELECT trans_id FROM po where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null'));");
                if (rs18.next()) {
                    po_inside_items= rs18.getInt(1);
                }
                db_qrs.close();
                ResultSet rs19 = db_qrs.select("SELECT count(po_item_id)  FROM po_item where trans_id in (SELECT trans_id FROM po where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null'));");
                if (rs19.next()) {
                    po_outside_items= rs19.getInt(1);
                }
                db_qrs.close();
                ResultSet rs20 = db_qrs.select("SELECT count(invoice_item_id)  FROM invoice_item where trans_id in (SELECT trans_id FROM invoice where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null'));");
                if (rs20.next()) {
                    inv__inside_items= rs20.getInt(1);
                }
                ResultSet rs21 = db_qrs.select("SELECT count(invoice_item_id)  FROM invoice_item where trans_id in (SELECT trans_id FROM invoice where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null'));");
                if (rs21.next()) {
                    inv__outside_items= rs21.getInt(1);
                }
                db_qrs.close();
                ResultSet rs24 = db_qrs.select("SELECT sum(total_price) FROM quote where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null');");
                if (rs24.next()) {
                    qte__inside_grand= rs24.getInt(1);
                }
                db_qrs.close();
                ResultSet rs25 = db_qrs.select("SELECT sum(total_price) FROM quote where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null');");
                if (rs25.next()) {
                    qte__outside_grand= rs25.getInt(1);
                }
                db_qrs.close();
                ResultSet rs26 = db_qrs.select("SELECT sum(total_price) FROM po where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null');");
                if (rs26.next()) {
                    po_inside_grand= rs26.getInt(1);
                }
                db_qrs.close();
                ResultSet rs27 = db_qrs.select("SELECT sum(total_price) FROM po where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null');");
                if (rs27.next()) {
                    po_outside_grand= rs27.getInt(1);
                }
                db_qrs.close();
                ResultSet rs28 = db_qrs.select("SELECT sum(total_price) FROM invoice where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key!='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key!='null');");
                if (rs28.next()) {
                    inv__inside_grand= rs28.getInt(1);
                }
                db_qrs.close();
                ResultSet rs29 = db_qrs.select("SELECT sum(total_price) FROM invoice where (from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null');");
                if (rs29.next()) {
                    inv__outside_grand= rs29.getInt(1);
                }
                db_qrs.close();
                ResultSet rs30 = db_qrs.select("SELECT count(invoice_id) FROM invoice where ((from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null') and (status='Payment Done'));");
                if (rs30.next()) {
                    no_of_payments= rs30.getInt(1);
                }
                db_qrs.close();
                ResultSet rs31 = db_qrs.select("SELECT sum(total_price) FROM invoice where ((from_regn_key=" + request.getParameter("nmbr") + " and to_regn_key='null') or (to_regn_key=" + request.getParameter("nmbr") + " and from_regn_key='null') and (status='Payment Done'));");
                if (rs31.next()) {
