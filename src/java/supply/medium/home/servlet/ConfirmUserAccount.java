 ...
 * @author LenovoB560
 */
public class ConfirmUserAccount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int result=UserMaster.activateUserAccountAndPassword(email, key, password);
            TestMemory.test("footer start");
                System.gc();
                TestMemory.test("footer end");
            if(result>0)
                response.sendRedirect("emailConfirmationResponse.jsp");
            else
