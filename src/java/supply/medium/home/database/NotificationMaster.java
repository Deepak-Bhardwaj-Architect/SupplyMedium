...
public class NotificationMaster {
    ...
    public static ArrayList showNotiifcationTypeCount(String userKey) {
        ...
        try {
            con = MyConnection.connect();
            st = con.createStatement();
            query = "select notification_type,count(*) from notification_master where status='Unread' and user_key_to='"+userKey+"' group by notification_type;";
    ...
    public static ArrayList showByStatus(String status, String userKey, String notificationType) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                nb.setNotificationOn(rs.getString(10));
                al.add(nb);
            }
        } catch (Exception ex) {
...
//                ErrorMaster.insert(query);
    ...
    public static int update(String notificationKey) {
        int result = 0;
        Connection con = null;
        Statement st = null;
        try {
            con = MyConnection.connect();
            st = con.createStatement();
            String query = "update notification_master set status='Read'"
