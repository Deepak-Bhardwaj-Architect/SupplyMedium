    ...
    public static ArrayList showCompaniesOutOfNetwork(String companyName, String businessKey, String companyKey, String vrType) {
        ...
        try {
            ...
            if (!businessKey.equals("0")) {
                query = "select  company_name,company_key from company_master where company_name like '%" + companyName + "%' and company_key not in('"+companyKey+"')  "
        ...
        try {
            ...
            if (!businessKey.equals("0")) {
                query = "select company_name,company_key "
                        + "from company_master "
                        + "where company_name like '%" + companyName + "%' and company_key not in('"+companyKey+"')  "
                        + "and company_type like '%" + vrType + "%' "
                        + "and company_key in "
                        + "(select company_key "
