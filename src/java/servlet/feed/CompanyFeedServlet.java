...
public class CompanyFeedServlet extends HttpServlet
{
	        ...
	        throws ServletException, IOException
	{
		...
		try
        {
			...
			//errorMaster_.insert( "companyfeedlistcount="+userFeedList.size( ) );
		...
		if( responseCode == 1600 || responseCode == 1610 || responseCode == 1620 || responseCode == 1625 )
		{
			...
			else
			{
				...
				for( int i=0;i<feedsCount;i++ )
                {
					...
					errorMaster_.insert( "Regn data.URL ="+ companyFeedData.userPictureUrl_ );
					
					errorMaster_.insert( "desc="+companyFeedData.feed_ );
					...
					else
					{
						...
						pathBuilder = null;
                                                webURL.value=companyFeedData.userPictureUrl_;
					}
					
					errorMaster_.insert( "After if else" );
