...
public class AdController
{
	...
	{
		errorMaster_.insert( "StoreImage");
		...
		try
        {
			...
			if( dirPathRes == 0 ) // Ad image directory path fetched successfully
			{
				...
				if( storeResult == 0 )  // user ad image stored in folder successfully
				{
					...
					adData.adImagePath_ = storedPath.value.replace(FileUploadPath.getFileUploadPath(request), AppConfigReader.instance( ).get( "TEMP_PATH" )+"/");
					//System.out.println("adData.adImagePath_////////"+adData.adImagePath_);
