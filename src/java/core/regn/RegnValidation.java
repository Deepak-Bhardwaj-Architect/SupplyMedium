...
public class RegnValidation
{
	...
	public int validate( String email, String companyName )
	{
		...
		try
		{
			...
			errorMaster_.insert("companyemaildomain="+comEmailDomainData.emailDomain_);
	...
	public boolean isCompanyIdExist( String companyId )
	{
		...
		try
		{
			...
			companyId = companyId.replaceAll("\\s","");
			
			result = companyRegnTblObj.isCompanyIdExist( companyId );
		...
		catch( Exception ex )
		{
			...
			errLogger.logMsg( "Exception::RegnValidation.isCompanyIdExist() - " + ex );
