...
public class UserGroupMapper
{
	...
	public int add( GroupKey key, List< UserProfileData > userProfileDataList )
	{
		...
		for( UserProfileData userProfileData :  userProfileDataList )
		{
			...
			//errorMaster_.insert( "Add user to group: GroupRelKey = "+key.groupName_+ ","+key.companyRegnKey_ );
	...
	public int getUsers( GroupKey groupKey,List<UserProfileData> userProfileDataArr )
	{
		...
		errorMaster_.insert( "GroupKey: "+groupKey.companyRegnKey_+":"+groupKey.groupName_);
		
		responseCode = find( groupKey, groupUsers );
		
		errorMaster_.insert( "Mapper - Response code = "+responseCode  );
		...
		errorMaster_.insert( "allUsers count="+allUsers.size( )
		...
		try
        {
			...
			errorMaster_.insert( "allusers="+allUsers.size( )+"groupuser="+groupUsers );
	...
	public int updatePrivileges( UserProfileKey profileKey )
	{
		...
		for( UserGroupMappingData userGroupMappingData : groupList )
        {
			...
			errorMaster_.insert( "priResult = " + priResult );
