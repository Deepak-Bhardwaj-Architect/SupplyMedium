...
public class PasswordHistory
{
	...
	private int updatePassword( PasswordResetData passResetData, PasswordPolicyData passwordPolicyData )
	{
		...
		if( passResult == 0 )
		{
			...
			if( passResult == 0 )
			{
				...
				errorMaster_.insert( "Max Value = "+max.value );
				errorMaster_.insert( "Passwordhistory = "+passwordPolicyData.passwordHistoryDays_ );
				if( max.value >= passwordPolicyData.passwordHistoryDays_ )
				{
					...
					errorMaster_.insert( "Removing function" );
	...
	private int remove( UserProfileKey userProfileKey )
	{
		...
		errorMaster_.insert( "Find result = "+result );

		if( result == 0 )
		{
			result = historyTable.delete( passResetData );
		
			errorMaster_.insert( "Delete result = "+result );
