...
public class CompanyFeedCache
{
	...
	public int put( CompanyRegnKey regnKey, SortedMap<Long, CompanyFeedData> feedMap )
	{
		...
		try
		{
			...
			errorMaster_.insert( "Department feed map count(1) ="+compFeedMap_.size( ) );
			...
			errorMaster_.insert( "Put the feeds to cahce fetched from database. count = "+feedMap.size( ) +"Regnkey="+regnKey);
	...
	public int put( CompanyRegnKey regnKey, CompanyFeedData compFeedData )
	{
		...
		try
		{
			...
			/*if( feedMap.size( ) >100 )
			{
				...
				errorMaster_.insert( "olderset key="+oldestKey );
			...
			errorMaster_.insert( "Department feed map count(2) ="+compFeedMap_.size( ) );
	        ...
	        int count )
	{
		...
		try
		{
			...
			if( feedMap == null )
			{
				errorMaster_.insert( "Company Feeds not available in cache." );
			...
			for( Map.Entry<Long, CompanyFeedData> entry : filtermap.entrySet( ) )
			{
				...
				errorMaster_.insert( "company feed id="+entry.getValue( ).companyFeedId_ );
			...
			errorMaster_.insert( "feedlist count="+compFeedList.size( ) );
			...
			if( compFeedList.size( ) != count )
			{
				...
				errorMaster_.insert( "All the feeds not available from company feed cache." );
			...
			errorMaster_.insert( "All the feeds fetched from company feed cache. Feeds count="+compFeedList.size( ) );
	...
	public int find( CompanyRegnKey regnKey, List<CompanyFeedData> compFeedList, long latestFeedId )
	{
		...
		try
		{
			errorMaster_.insert( "deptkey="+regnKey );
			
			errorMaster_.insert( "Dept map count="+compFeedMap_.size( ));
			...
			errorMaster_.insert( "Company feeds count="+feedMap.size( ) );
			
			SortedMap<Long, CompanyFeedData> filtermap = feedMap.headMap( latestFeedId );
			
			errorMaster_.insert( "filter map="+ filtermap);
			...
			for( Map.Entry<Long, CompanyFeedData> entry : filtermap.entrySet( ) )
			{
				...
				errorMaster_.insert( "user feed id="+entry.getValue( ).companyFeedId_ );
