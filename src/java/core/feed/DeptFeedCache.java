...
public class DeptFeedCache
{
	...
	public int put( DeptKey deptKey, SortedMap<Long, DeptFeedData> feedMap )
	{
		...
		try
		{
			...
			errorMaster_.insert( "Department feed map count(1) ="+deptFeedMap_.size( ) );
			...
			errorMaster_.insert( "Put the feeds to cahce fetched from database. count = "+feedMap.size( ) +"Deptkey="+deptKey);
	...
	public int put( DeptKey deptKey, DeptFeedData deptFeedData )
	{
		...
		try
		{
			...
			errorMaster_.insert( "Department feed map count(2) ="+deptFeedMap_.size( ) );
	        ...
	        int count )
	{
		...
		try
		{
			...
			if( feedMap == null )
			{
				errorMaster_.insert( "Department Feeds not available in cache." );
			...
			for( Map.Entry<Long, DeptFeedData> entry : filtermap.entrySet( ) )
			{
				...
				errorMaster_.insert( "user feed id="+entry.getValue( ).deptFeedId_ );
			...
			errorMaster_.insert( "feedlist count="+deptFeedList.size( ) );
			...
			if( deptFeedList.size( ) != count )
			{
				...
				errorMaster_.insert( "All the feeds not available from department feed cache." );
			...
			errorMaster_.insert( "All the feeds fetched from dept feed cache. Feeds count="+deptFeedList.size( ) );
	...
	public int find( DeptKey deptKey, List<DeptFeedData> deptFeedList, long latestFeedId )
	{
		...
		try
		{
			errorMaster_.insert( "deptkey="+deptKey );
			
			errorMaster_.insert( "Dept map count="+deptFeedMap_.size( ));
			...
			errorMaster_.insert( "department feeds count="+feedMap.size( ) );
