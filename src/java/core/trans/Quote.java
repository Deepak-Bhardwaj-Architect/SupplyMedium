	...
	private int sendQuoteMail( QuoteData quoteData )
	{
		...
		try
        {
			...
			if( quoteData.isOutsideSupplier_ == 1&& quoteData.outsideSupplierEmailFlag_ == 1 )
			{
				...
				String subject = "Quote (QTE"+quoteData.transId_+") from "+quoteData.fromCompanyProfileData_.companyName_;
				...
				//String message = composer.composeTransactionMail( trasnMsg );
                                String message = composer.composeTransactionMail3( trasnMsg,quoteData);
			...
			{
				to[0] = quoteData.userTo_.toString();
