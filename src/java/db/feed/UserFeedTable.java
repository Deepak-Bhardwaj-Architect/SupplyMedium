...
if( errorMaster_ == null ) errorMaster_ = new ErrorMaster( );
	...
	public int find( UserProfileKey userKey, List<UserFeedData> userFeedList,int from,int count )
	{
		...
		query = "SELECT * FROM " + tableName_ + " "
                        + "WHERE user_rel_key = '" + userKey.toString( ) + "' "
...
//                        query +=  " ORDER BY created_timestamp DESC LIMIT "+from+", "+count;
                        query="select * from user_feed where regn_rel_key in "
                                + "(SELECT regn_key FROM company_registration "
                                + "where business_category_name in "
                                + "(SELECT business_category_name "
                                + "FROM company_registration where regn_key in "
                                + "(select regn_rel_key from user_feed "
                                + "where user_rel_key='"+userProfileKey.toString( )+"'))) "
...
//                                query += " UNION ALL ( ";
                        ...
                        query = "select * from user_feed where regn_rel_key in "
                                ...
                                + "(SELECT business_category_name "
                                + "FROM company_registration where regn_key in "
                                + "(Select regn_rel_key FROM "+ tableName_ + " "
                                + "WHERE user_rel_key ='"+userProfileKey.toString( )+"' "
                                + "AND user_feed_id > "+lastFeedId+"))) "
                                + "ORDER BY created_timestamp DESC ";
