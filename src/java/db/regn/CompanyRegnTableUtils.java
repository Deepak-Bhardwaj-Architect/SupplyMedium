...
public class CompanyRegnTableUtils
{
	...
	{
//		System.out.println("queryFilter.country_"+queryFilter.country_);
		int result = 0;//-1;
		...
		if( queryFilter.signUpAs_ != "" && queryFilter.signUpAs_ != null )
		{
			...
			whereClause.value += " OR company_type = 'Buyers/Suppliers' )";
                      
		}
                
                
                if(queryFilter.country_ != "" && queryFilter.country_ != null && !queryFilter.country_.equals( "All" ))
		{
                    if( whereClause.value != "" )
			{
				whereClause.value += " AND";
			...
			whereClause.value += " regn_key in ( select regn_key from mailing_address where country_name='"+queryFilter.country_+"' ) ";
                        //result = 0;
                }
		...
		if( queryFilter.businessCategory_ != "" && queryFilter.businessCategory_ != null && !queryFilter.businessCategory_.equals( "All" ))
		{
			...
			whereClause.value += " business_category_name like '%" + queryFilter.businessCategory_ + "%'" ;
			...
			//result = 0;
               ...
               // System.out.println("qry="+queryFilter.country_+whereClause.value);
		 
                ErrorMaster errorMaster_ = null;

                if( errorMaster_ == null ) errorMaster_ = new ErrorMaster( );
                        errorMaster_.insert( "where condition ="+ whereClause.value+result);
		...
		try
		{
			while( rs.next( ) )
			{
				...
				companyKeys.add( companyRegnKeyObj );
                                //System.out.println(companyRegnKeyObj.companyPhoneNo_);
			}
