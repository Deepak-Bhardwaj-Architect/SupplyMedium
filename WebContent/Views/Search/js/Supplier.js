...
function SerachResultContainor(vendor,autoNumber, regnKey,className,noOfRatings,avgRatings )
{
	var ratings;
	...
	var SerachResult=
		'<div class="searchRow '+className+'" id= '+regnKey+'>'
		...
		+'<div class="searchCol3"><ul>'
				...
				+'<li class="searchCmpAddr"></li>'
				+'<li><span class="searchRattings">'+ratings+'</span></li>'
			+'</ul></div>'
		+'</div>';
...
function basicSearch()
{
	...
	$.ajax({
		type : "POST",
		url : getBaseURL() + "/SearchSupplierServlet",
		data : {
			'SearchType' : 'BasicSearch',
			'RegnKey' : regnKey,
			'Category' : category,
				...
				if( vendorArr.length == 0 )
			  	...
			  	for( var i=0; i<vendorArr.length; i++ )
				  {
				  		...
				  		if( vendor.isRegn == "true" )
				  		{
				  			...
				  			$("#regSearchResult .mCSB_container").append(SerachResultContainor(vendor,j, vendor.regnKey,className,vendor.noOfRatings,vendor.avgRatings));
				  		...
				  		else
				  		{
				  			...
				  			$("#nonregSearchResult .mCSB_container").append(SerachResultContainor(vendor,k, vendor.regnKey,className,vendor.noOfRatings,vendor.avgRatings));
				 ...
				 var countString = "";
				  ...
				  j==1? (countString = " Supplier found"):(countString = " Suppliers found");
				  
				  $("#reg_comp_count").text( "( "+ j + countString +" )" );
				  
				  k==1? (countString = " Supplier found"):(countString = " Suppliers found");
...
function advanceSearch()
{
	...
	if ($('#regSupplierCheck').is(':checked'))
	{
		regnSupplier = '1';
	} 
	else
	...
	advanceSearchObj.buyerKey = regnKey;
	advanceSearchObj.checkAll  = checkAll;
	advanceSearchObj.searchRegd  = regnSupplier;
...
/* This will load the empty RFQ form when a Registered supplier is selected
 * as a result of basic search and generate button is clicked
 * 
 * Then, the Company Name of the selected company will be filled in the RFQ form
 * */
function loadBasic( )
{
		...
		$( "#to_company" ).val( selectedCompanyName ); 
		$( "#selected_ven_key" ).val( selectedRegnkey );
		getRFQSupplierAddress( selectedRegnkey );
 ...
 * 
 * 
 *  Then, the items entered in the Advanced search will be filled in the RFQ form
 */
function loadAdvanced( )
{
	...
	for(var i=0;i<searchItemCount;i++ )
	{
		...
		itemDet += '<input type="text" class="textbox" id="sno'+itemsCount+'" style="width:30px;margin-right:20px;"  value="'+sNo+'";/>';
		
		itemDet += '<input type="text" class="textbox" id="item_desc'+itemsCount+'" style="width:175px;margin-right:20px;"   value="'+searchItem.name+'";/>';
		itemDet += '<input type="text" class="textbox" id="part_no'+itemsCount+'" style="width:90px;margin-right:20px;"  value="'+searchItem.partNo +'";/>';
		itemDet += '<input type="text" class="textbox" id="quantity'+itemsCount+'" style="width:90px;"  value="";/>';
		itemDet += '<div class="quantity_unit" id="quantity_unit'+itemsCount+'" >KG</div>';
		itemDet += '<div class="quan_units" id="units_quantity_unit'+itemsCount+'" style="display:none;left:487px;"></div>';
		itemDet += '<input type="text" class="textbox" id="ship_date'+itemsCount+'" style="width:110px;margin-right:20px;"  value="";/>';
...
function displayNoResult( displayType )
{
	...
	{		
		...
		var divRow = '<div class="searchRow" style="text-align:center; width:500px; font-size:16px;font-color:#464646;">No registered suppliers found for this search criteria</div>';
