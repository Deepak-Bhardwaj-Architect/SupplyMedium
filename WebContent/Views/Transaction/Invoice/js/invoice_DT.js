...
function showInvoicePopupForm( aData )
{
	...
	var invoiceNo = aData[32];
	var poNo	= aData[33];
	var invoiceDuedate = aData[34];
	var freightHandling = aData[35];
...
function parseInvoiceRequest( invoiceArr )
{
	...
	for( var i=0;i<invoiceArr.length;i++)
	{
		...
		invoiceDataTable.fnAddData([transImg,InvoiceId,InvoiceDate,fromCompName,fromRegnKey,fromUserKey,fromState+inquireChat,status,
				...
				carrier,billOfLanding,freightWeight,freightWeightUnit,dateShipped,isDiffAddr,isNonPoInvoice,diffEmail]);
	}
