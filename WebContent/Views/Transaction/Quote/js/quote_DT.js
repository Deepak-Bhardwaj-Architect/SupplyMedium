...
function showQuotePopupForm( aData )
{
	...
	var fromState= aData[6];
	var status= aData[7];
	var fromCountry= aData[9];
	var fromCity= aData[10];
	var fromAddress= aData[11];
	var fromZipcode= aData[12];
	var toRegnKey= aData[13];
	var toUserKey= aData[14];
	var toCompName= aData[15];
	var toCountry= aData[16];
	var toState= aData[17];
	var toCity= aData[18];
	var toAddress= aData[19];
	var toZipcode= aData[20];
	var isOutsideSupplier= aData[21];
	var outsideSupplierEmail= aData[22];
	var recurring= aData[23];
	var quotationRef= aData[24];
	var items= aData[25];
	var transaction= aData[26];
	var inquires= aData[27];
	var transId= aData[28];
	
	var totListPrice = aData[29];
	var tax = aData[30];
	var totPrice = aData[31];
...
function parseQuoteRequest( quoteArr )
{
	...
	for( var i=0;i<quoteArr.length;i++)
	{
		...
		quoteDataTable.fnAddData([transImg,QuoteId,QuoteDate,fromCompName,fromRegnKey,fromUserKey,fromState+inquireChat,status,
				...
				items,trans,inquires,transId,totListPrice,tax,totPrice,isPOCreated]);
	}
