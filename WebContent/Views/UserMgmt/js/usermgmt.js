...
function addCtrlBtns( rowNo,userId )
{
	...
	var buttons="<img src='Views/UserMgmt/images/user_edit_btn.png'style='cursor:pointer;z-index:100;margin-right:10px;' title='Update User Settings' onclick='openEditView("+rowNo+")'/>";
...
function addStatusDropDown( status,rowNo,userId )
{
	...
	if( status == "Pending" )
	{
		...
		selectBox = selectBox + "<option>Reject</option>";
		...
		$.ajax({
			   ...
			   success: function( data ) 
			   {
				  ...
				  else
				  {
					  ...
					  $("#newusererr").text(data.message);
				  }
			   },
			    error : function(xhr, textStatus, errorThrown) 
			    {
			    	$("#newusererr").text("Unexcepted error occur. Please try again.");
