...
function showInvoicePopupForm( aData )
{
	...
	var isDiffAddr = aData[41];
	var isNonPOInvoice = aData[42];
	var diffEmail = aData[43];
	...
	for(var i=0;i<items.length;i++)
	{
		...
		itemDet += '<input type="text" class="textbox" id="popup_quantity_ordered'+sNo+'" style="width:90px;" disabled value="'+item.quantityOrdered+'";/>';
		...
		itemDet += '<input type="text" class="textbox" id="popup_quantity_shipped'+sNo+'" style="width:90px;" disabled value="'+item.quantityShipped+'";/>';
...
function parseInvoiceRequest( invoiceArr )
{
	...
	for( var i=0;i<invoiceArr.length;i++)
	{
		...
		var fromState = invoice.invoiceTotal;
		...
		var isOutsideSupplier = invoice.isOutsideSupplier;
		...
		var status = invoice.status;
                ...
                var open="<a title='After payment done file will be downloaded'><img src='Views/Transaction/TransCommon/images/open.png'/></a>";
		
		if( status == "InvoiceAccepted" ||status == "Payment Done" )
		{
                   // alert(fromRegnKey+" nd "+regnKey)
                    //if(regnKey!==undefined)
                    //{
                    if( status === "InvoiceAccepted"&&fromRegnKey !== regnKey)
		{
                       // alert('enter');
			status = "<input type='button' id='prcd_pymnt" + InvoiceId + "' value='Proceed to Pay' class='generate_trans_btn' onclick='proceedpayment(" + InvoiceId + ","+totPrice+");'>";
                }
                else if( status === "InvoiceAccepted"&&fromRegnKey === regnKey)
                {
                    status ="Pending";
                }    
                   // }
                ...
                {
                    open="<a onclick=\"javascript:this.href='payment_pdf_download?trans_id=" + invoice.transId + "&from=" + fromCompName + "&to=" + toCompName + "&date=" + InvoiceDate + "&typ=" + trans_typ +"'\"  target='_blank'><img src='Views/Transaction/TransCommon/images/open.png'/></a>";
                }
                var trans_typ="";
		...
		if (fromRegnKey == regnKey)
        {
            ...
            invoiceDataTable.fnAddData([transImg,InvoiceId,InvoiceDate,toCompName,toRegnKey,toUserKey,fromState+inquireChat,status,open,
				...
				carrier,billOfLanding,freightWeight,freightWeightUnit,dateShipped,isDiffAddr,isNonPoInvoice,diffEmail,"sent"]);
        }
        else
        {
            invoiceDataTable.fnAddData([transImg,InvoiceId,InvoiceDate,fromCompName,fromRegnKey,fromUserKey,fromState+inquireChat,status,open,
				...
				toCity,toAddress,toZipcode,isOutsideBuyer,outsideBuyerEmail,recurring,invoiceRef,
				items,trans,inquires,transId,totListPrice,tax,totPrice,invoiceNum,poNum,invoiceDueDate,freightHandling,
				carrier,billOfLanding,freightWeight,freightWeightUnit,dateShipped,isDiffAddr,isNonPoInvoice,diffEmail,"rec"]);
        }
...
function proceedpayment(val,price)
{
    $('#amnt').val(price);
    $('#rtrn').val($('#rtrn').val()+'trns'+val);
    //alert($('#amnt').val()+"nd"+$('#rtrn').val());
