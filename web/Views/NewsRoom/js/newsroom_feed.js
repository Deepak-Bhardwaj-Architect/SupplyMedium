...
function postFeed() {       
	...
	data.append('UserKey', userKey);
        //alert(document.user_feed_frm.new_feed_image.files[0]);    
	...
	$.ajax({
		...
		success : function(data) 
		{
			...
			if (data.result == "success")
			{
				...
				data.feeds[0].userName = $("#firstName").val()+" "+$("#lastName").val();
					
				data.feeds[0].email = $("#EmailAddress").val();
					
				data.feeds[0].userPictureURL = $("#user_image_url").val();
...
function fetchNewsRoomLatestFeeds() {
	...
	//alert("latest feed id="+latestFeedId);
	...
	$.ajax({
		type : "POST",
		url : getBaseURL() + "/NewsFeedServlet",
		data : {
...
function deleteConfirmation( feedId )
{
	deleteFeedId = feedId;
	
	showWarning( "Do you want to delete this post?" );
	...
	$.ajax({
		...
		success : function(data)
		{
			...
			if (data.result == "success")
			{
				...
				$("#feed_"+deleteFeedId).slideUp("slow",function ()
				{
					$("#feed_" + deleteFeedId).remove();
...
function placeFeedsTop(feeds, position) 
{     
	var userKey = $('#EmailAddress').val();
	...
	for ( var i = 0; i < feeds.length; i++) 
	{
		...
		var userPictureURL = feed.userPictureURL;
                userPictureURL=userPictureURL.replace("http://localhost:8080//","");
		...
		var feedContent = "<div id='feed_"
				+ feedId
				+ "' class='feed' style='display:none;'>"; /// already tested
		...
		if( userKey == email )
		{
			...
			+ feedId + "' onclick='deleteConfirmation(" + feedId
			+ ")' style='display:none;'/>";
		}
		...
		$("#feed_" + feedId).on('mouseleave', function(event) 
		{
			...
			if (idSplitArr.length > 1) 
			{
                ...
                $('#thumb_'+feedId).css('display','block');
                $('#thumb_'+feedId).show();
	...
	for ( var i = 0; i < feeds.length; i++) 
	{
		...
		if (imageURL != "null")
		{
			if((imageURL.substring(imageURL.lastIndexOf(".")+1)==="xls"||(imageURL.substring(imageURL.lastIndexOf(".")+1)==="xlsx")))
                        {
                         feedContent = feedContent + "<a href='"+imageURL+"' target='_blank'><img src='http:\\\\localhost:8080\\SupplyMedium\\Views\\NewsRoom\\images\\excel.png'  id='thumb_"+feedId+"' height='40'/></a>";
			  $("#thumb_"+feedId).hide();   
                        }
                        else if((imageURL.substring(imageURL.lastIndexOf(".")+1)==="doc"||(imageURL.substring(imageURL.lastIndexOf(".")+1)==="docx")))
                        {
                            feedContent = feedContent + "<a href='"+imageURL+"' target='_blank'<img src='http:\\\\localhost:8080\\SupplyMedium\\Views\\NewsRoom\\images\\word.png'  id='thumb_"+feedId+"' height='40'/></a>";
			    $("#thumb_"+feedId).hide(); 
                        }
                        else if((imageURL.substring(imageURL.lastIndexOf(".")+1)==="pdf"))
                        {
                            feedContent = feedContent + "<a href='"+imageURL+"' target='_blank'<img src='http:\\\\localhost:8080\\SupplyMedium\\Views\\NewsRoom\\images\\pdf.png'  id='thumb_"+feedId+"' height='40'/></a>";
			    $("#thumb_"+feedId).hide(); 
                        }
                        else
                        {
		...
		feedContent += "<p class='feed_text'>" + feedDesc + "<p>";

		feedContent += "</div>";
                feedContent +="<div id='mn_"+feedId+"' style='height:0px;margin-top:-50px;'>";
                feedContent +="</div>";
                //alert('1');
                likes_comments_val(feedId);
		//alert('2');
		if( parseInt(feedId) > latestFeedId )
		{
...
/****************************get like and comment of newsroom***************************/

function likes_comments_val( id ) 
{
    //alert('ok');
    var feedContent="";
    var lk_cmnt=null;
