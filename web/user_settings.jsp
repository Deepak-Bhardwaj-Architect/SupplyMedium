...
<%
    ...
    try {
        ...
        rs = statement.executeQuery("SELECT user_profile.first_name,user_profile.last_name,user_profile.phone,user_profile.cell,mailing_address.city_name,mailing_address.state_name,mailing_address.zipcode,mailing_address.country_name,user_profile.department,user_profile.user_role,user_profile.fax,user_profile.user_type FROM user_profile INNER JOIN mailing_address ON user_profile.email=mailing_address.email where user_profile.email='" + request.getParameter("id") + "'");
        if (rs.next()) {
%>
    ...
    <div class="contentdetail" id="content_detail" style="padding:10px;width:100%;height: auto;border-right:2px solid;border-right-color: #A4A4A4;">
        ...
        <div class="row">
            ...
            <select class="textbox"  style="height: 28px;width:190px;" name="usr_tp" id="usr_tp" >                                        
                <option <% if(rs.getString(12).equals("Intranet User")){ out.print("selected"); } %>>Intranet User</option>
                <option <% if(rs.getString(12).equals("Regular")){ out.print("selected"); } %>>Regular</option>
            </select>                                    
        </div>   
        <div class="row">
...
<div style="width:50%;height:92%;float:left;">
    <div class="popupcontent" style="/*border-bottom-color: #A4A4A4;*/ padding:10px;margin-top:0px;">
        ...
        <!--div style="float: left;width: 100%;padding:10px;margin-top:10px;">
            <div class="grouppriv"><!-- comment it to show the jalwa-->
                <%
               /* Statement statement2 = null;
    Connection con2 = null;
    ResultSet rs2 = null;
    try {
        con2 = DBConnect.instance().getConnection();
        statement2 = con2.createStatement();
        rs2 = statement2.executeQuery("SELECT privileges FROM user_privileges where user_rel_key='" + request.getParameter("id") + "'");
        if (rs2.next()) {
        String privileges=rs2.getLong("privileges")+"";
        while(privileges.length()<14)
        {
         privileges="0"+privileges;   
                ...
                <fieldset class="privfieldset">
                    ...
                    <div class="privleft" style="width: 40%;float:left; ">
                        <div class="listitem" style="float: left">
                            <div class="checkContainer"><input type="checkbox" class="checkbox" id="addusers" <% if(privileges.length()==14){ if(privileges.substring(0,1).equals("1")) {out.print("checked");} } %> /><div></div></div>
                            <label for="addusers" class="group_pri_lbl">Add New Users</label>

                        </div>
                        <div class="listitem" style="float: left">
                            <div class="checkContainer"><input type="checkbox" class="checkbox"  id="delusers" <% if(privileges.length()==14){ if(privileges.substring(1,2).equals("1")) {out.print("checked");} } %>><div></div></div>
                            <label for="delusers"  class="group_pri_lbl">Delete Users</label>
                        </div>                        
                        <div class="listitem" style="float: left">
                            <div class="checkContainer"><input type="checkbox" class="checkbox"  id="addbuyers"  <% if(privileges.length()==14){ if(privileges.substring(4,5).equals("1")) {out.print("checked");} } %>><div></div></div>
                            <label for="addbuyers" class="group_pri_lbl">Add Buyers</label>
                        </div>                       
                        <div class="listitem" style="float: left">
                            <div class="checkContainer"><input type="checkbox" class="checkbox"  id="accessusermgmt"  <% if(privileges.length()==14){ if(privileges.substring(6,7).equals("1")) {out.print("checked");} } %>><div></div></div>
                    ...
                    <div class="privright" style="width:40%; float:right;">
                        ...
                        <div class="listitem" style="float: left">
                            <div class="checkContainer"><input type="checkbox" class="checkbox"  id="postanncemnt"  <% if(privileges.length()==14){ if(privileges.substring(7,8).equals("1")) {out.print("checked");} } %>><div></div></div>
                            <label for="postanncemnt" class="group_pri_lbl">Post Announcements</label> 
                        </div>
                        <div class="listitem" style="float: left">
                            <div class="checkContainer"><input type="checkbox" class="checkbox"  id="rate"  <% if(privileges.length()==14){ if(privileges.substring(9,10).equals("1")) {out.print("checked");} } %>><div></div></div>
                            <label for="rate" class="group_pri_lbl">Rate Buyers/Suppliers</label>
                        </div>
                        <div class="listitem" style="float: left">
                            <div class="checkContainer"><input type="checkbox" class="checkbox"  id="creategroup"  <% if(privileges.length()==14){ if(privileges.substring(10,11).equals("1")) {out.print("checked");} } %>><div></div></div>
                            <label for="creategroup" class="group_pri_lbl">Create User Group</label>
                        </div>
                        <div class="listitem" style="float: left">
                            <div class="checkContainer"><input type="checkbox" class="checkbox"  id="delgroup"  <% if(privileges.length()==14){ if(privileges.substring(11,12).equals("1")) {out.print("checked");} } %>><div></div></div>
                            <label for="delgroup" class="group_pri_lbl">Delete User Group</label>
                        </div>                     
                    </div>
                    <div class="groupupt" style="margin-top:80px;margin-left:30%;">
                        <br><input type="button"  id="updatepribtn" class="gen-btn-Orange" value="Update"  onclick="update_user_privilages('<%=request.getParameter("id") %>')"/> 
                    </div>
