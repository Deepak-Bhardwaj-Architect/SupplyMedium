...
function showRFQPopupForm(aData)
{
   ...
   // alert(items);
    ...
    //alert(aData[31]);
    $('#rfq_tl').text("RFQ  date: "+RFQDate)
    $('#typ').val(aData[31]);
    $('#outside_supplier_address').val(aData[32]);
    ...
    for (var i = 0; i < items.length; i++)
    {
        ...
        itemDet += '<input type="text" autocomplete="off" class="textbox" id="popup_item_desc' + sNo + '" style="width:175px;margin-right:20px;" disabled  value="' + item.itemDesc + '";/>';
        ...
        itemDet += '<input type="text" autocomplete="off" class="textbox" id="popup_ship_date' + sNo + '" style="width:110px;margin-right:20px;" disabled value="' + item.shipDate + '";/>';
...
function parseRFQRequest(rfqArr)
{
    ...
    for (var i = 0; i < rfqArr.length; i++)
    {
        ...
        if(isOutsideSupplier==="1")
                {
                   ...
                   var outsideSupplieraddress=rfq.outsideSupplieraddress.split(",")
        ...
        if (fromRegnKey == regnKey)
        {
            ...
            rfqDataTable.fnAddData([transImg, RFQId, RFQDate, toCompName , toRegnKey, toUserKey, toState, status, open,
                ...
                items, trans, inquires, transId, isQuoteCreated,"sent",outsideSupplieraddressdetail]);
        }
        else
        {
           rfqDataTable.fnAddData([transImg, RFQId, RFQDate, fromCompName, fromRegnKey, fromUserKey, fromState + inquireChat, status, open,
                ...
                toCity, toAddress, toZipcode, isOutsideSupplier, outsideSupplierEmail, recurring, quotationRef,
                items, trans, inquires, transId, isQuoteCreated,"rec",outsideSupplieraddressdetail]); 
        }
    }
