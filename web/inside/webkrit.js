...
{
    $('#' + val).hide();
//    $("#screen_lock").attr('disabled','disabled');
//    $("#screen_lock").prop('disabled',true);
//    $("#screen_lock").prop('disabled','disabled');
...
function selectUserRow(select, userKey, status)
{
    ...
    var actionImages = "<img src='inside/user_edit_btn.png' style='cursor:pointer;z-index:100;margin-right:10px;' title='Update User Settings' onclick=\"javascript:showUserSettingForAdmin('" + userKey + "');\">" +
            "<img src='inside/user_delete_btn.png' style='cursor:pointer;z-index:101;' title='Delete User' onclick=\"javascript:$('#warning_container').show();\">";
...
function addUpdateDeleteCompanyMultipleAddress()
{
    ...
    if (companyAddressCount === 1)
    {
        ...
        $("#addrleft2").show();
        $('#compseparator').css({'height': '475px', 'margin-bottom': '50px'});
        $('#canBtnDiv').css({'top': '165px'});
        companyAddressCount++;
    ...
    else if (companyAddressCount === 2)
    {
        ...
        $('#compseparator').css({'height': '495px', 'margin-bottom': '50px'});
        $('#canBtnDiv').css({'top': '185px'});
        companyAddressCount++;
        $('#companyAddressCount').val(companyAddressCount);
...
function removeUpdateDeleteCompanyMultipleAddress(value)
{
    ...
    else if (companyAddressCount === 2 && value === 2)
    {
        ...
        $("#addrleft1").show();
        $("#address2detail").hide();
        $('#compseparator').css({'height': '375px', 'margin-bottom': '100px'});
        $('#canBtnDiv').css({'top': '115px'});
        companyAddressCount--;
        $('#companyAddressCount').val(companyAddressCount);
        $('#addAddtessButton').show();
    }
...
//    alert(selectItem+":"+addItem);
    if (selectItem == true)
    {
        ...
        itemDet += '<div id="item' + itemsCount + '" class="item" style="width:900px;float:left;margin-left:40px;margin-right:10px;">';
        ...
        itemDet += '<input type="button" class="del_btn" id="del_btn_' + itemsCount + '" style="width:110px;margin-right:10px;" onclick=$("#item' + itemsCount + '").html("");document.getElementById("allRfQItems").value=document.getElementById("allRfQItems").value.replace("(' + lblkey + '^' + lblpn + '^' + lblbc + '^' + quantity + '^' + quantityType + '^' + shipDate + '),",""); /></div>';
    ...
    else if (addItem == true)
    {
        ...
        if(lblpn.trim()==="")
        {
          $('#rfq_item_form_error2').html("Enter Part No");
            return false;  
        }
        if(lblsn.trim()==="")
        {
...
//    alert(selectItem+":"+addItem);
    if (selectItem == true)
    {
        ...
        itemDet += '<input type="hidden" autocomplete="off" class="textbox" name="quantityUnitKey" id="itemKey' + itemsCount + '" style="width:30px;margin-right:10px;" readonly value="1"/>';
        ...
        itemDet += '<input type="hidden" autocomplete="off" class="textbox" name="currencyKey" id="itemKey' + itemsCount + '" style="width:30px;margin-right:10px;" readonly value="1"/>';
...
//        }
        ...
        itemDet += '<input type="text" autocomplete="off" class="textbox" maxlength=7 name="quantity" id="quantity_ordered' + itemsCount + '" style="width:28px;" readonly value="' + popup_quantity_ordered + '"/>';
        itemDet += '<div class="quantity_unit" id="quantity_ordered_unit' + itemsCount + '" style="margin-right:10px;" >' + quantity_ordered_unit + '</div>';
        itemDet += '<input type="text" autocomplete="off" class="textbox" maxlength=7 name="quantityShipped" id="quantity' + itemsCount + '" style="width:28px;" readonly value="' + popup_quantity + '"/>';
        ...
        itemDet += '<input type="text" name="discount" autocomplete="off" class="textbox" id="multipl' + itemsCount + '" style="width:48px;margin-right:10px;" readonly value="' + dscnt + '"/>';
        itemDet += '<input type="text" name="total"  autocomplete="off" class="textbox" id="totl' + itemsCount + '" style="width:65px;margin-right:10px;" readonly value="' + pop_multiplier + '"/>';
        itemDet += '<input type="button" class="del_btn" id="del_btn_' + itemsCount + '" style="width:110px;margin-right:10px;" onclick=delPrice(' + pop_multiplier + ');$("#item' + itemsCount + '").html("");document.getElementById("allQteItems").value=document.getElementById("allQteItems").value.replace("(' + lblkey + "^" + lblpn + '^' + lblbc + "^" + popup_quantity_ordered + '^' + quantity_ordered_unit + '^' + popup_quantity + '^' + quantity_shipped_unit + "^" + popup_price + "^" + currency + "^" + dscnt + '),",""); /></div>';
        ;
...
function quoteFormValidation()
{
    ...
    else
    {
        if ($('#selectedVendorKey').val() === '0')
        {
            document.getElementById('quote_form_error').innerHTML = "Select company from search company list before send RFQ";
            document.getElementById('quote_form_error').style.display = 'block';
            $('#to_company').focus();
            return false;
        }
    ...
    {
        document.getElementById('quote_form_error').innerHTML = "Accept terms & conditions before send RFQ";
        document.getElementById('quote_form_error').style.display = 'block';
        $('#rfq_terms_cond').focus();
        return false;
    }
...
function poFormValidation()
{
    ...
    else
    {
        if ($('#selectedVendorKey').val() === '0')
        {
            document.getElementById('po_form_error').innerHTML = "Select company from search company list before send RFQ";
            document.getElementById('po_form_error').style.display = 'block';
            $('#to_company').focus();
            return false;
        }
    ...
    {
        document.getElementById('po_form_error').innerHTML = "Accept terms & conditions before send RFQ";
        document.getElementById('po_form_error').style.display = 'block';
        $('#po_terms_cond').focus();
        return false;
    }
...
function invoiceFormValidation()
{
    ...
    else
    {
        if ($('#selectedVendorKey').val() === '0')
        {
            document.getElementById('invoice_form_error').innerHTML = "Select company from search company list before send RFQ";
            document.getElementById('invoice_form_error').style.display = 'block';
            $('#to_company').focus();
            return false;
        }
    ...
    {
        document.getElementById('invoice_form_error').innerHTML = "Accept terms & conditions before send RFQ";
        document.getElementById('invoice_form_error').style.display = 'block';
...
setTimeout(showGlobalNotificationCount(), 1000);
...
//    last_selected_connection4 = userKey;
    $.ajax({
        ...
        success: function(data)
        {
            ...
            setTimeout(showUpdatedMessages(userKey),1000);
            //$('#search_member_text').val(first_name + " " + last_name + " (" + email + ")");
        },
        error: function(xhr, textStatus, errorThrown)
        {
...
//        alert('4');
        ...
        var result = "<div class='chat-window' id='chat-box-" + id + "' style='max-height:231px;right:" + (240 * chatboxes) + "px;'>\n\
                        <div class='chat-window-title' onclick=\"$('#chat-content-" + id + "').toggle()\">\n\
                            <div class='text'>" + name + "<div class='close' onclick='javascript:closeChatBox(\"" + id + "\");$(\"#chat-box-" + id + "\").remove();'></div></div>\n\
                        </div>\n\
                        <div class='chat-window-content' id='chat-content-" + id + "'>\n\
                            ...
                            <div style='bottom: 0;position: fixed;'>\n\
                                ...
                                <input type='button' onclick=\"javascript:saveMessage();\" value='Send'> \n\
                            </div>\n\
                        </div>\n\
                    </div>";
...
function send_private_message()
{
   ...
   $('#message').val('') ;
   $('#message').focus();
  // var message_content='<div class="messagecontainer"><div class="message_left"><img src="'+image+'" class="msg_img" id="msg_img"></div><div class="message_right"><div class="msg_name">lokesh kakkar</div><div class="msg_text"><p>'+message+'</p><p>sent on : 2014-11-18 22:34:29.0</p></div></div></div>';
    //$('#mCSB_container').append(message_content); 
    //$('#mCSB_container').scrollTop();
        $.ajax({
        type: "POST",
        dataType: 'text',
        url: "Messages",
        data:
                {
                    'userKeyTo': user_key,'message':message
                },
        success: function(data)
        {
            //alert(data);
