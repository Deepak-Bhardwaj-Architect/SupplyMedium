    ...
    if (chatids.length === i)
    {
        ...
        var result = "<div class='chat-window' id='chat-box-" + id + "' style='max-height:231px;right:" + (240 * chatboxes) + "px;'>\n\
                        <div class='chat-window-title' onclick=\"$('#chat-content-" + id + "').toggle()\">\n\
                            <div class='text'>" + name + "<div class='close' onclick='javascript:$(\"#chat-box-" + id + "\").remove();closeChatBox(\"" + id + "\",\"" + id + "\", \"" + name + "\");'></div></div>\n\
                        </div>\n\
...
function reArrangeChatBoxes()
{
    ...
    {
        $('#chat-box-'+c_boxes[i]).css('z-index', '2').animate({'right':(240 * i)+"px"});
        $('#chattext'+c_boxes[i]).css('z-index', '3').animate({'right':(240 * i)+"px"});
        $('#chatbtn'+c_boxes[i]).css('z-index', '4').animate({'right':(240 * i)+"px"});
        $('#chat-content-' + c_boxes[i]).toggle();
        $('#chat-box-'+c_boxes[i]).animate({right:(240*i)+"px"},0);
    }
}
