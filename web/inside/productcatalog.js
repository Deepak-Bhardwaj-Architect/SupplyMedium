			...
			if (data.result == "success")
			{
				for ( var i = 0; i < data.CatalogList.length; i++)
				{
					...
					var ItemSKU = CatalogItem.itemsku_;
                                        var barcode = CatalogItem.barcode_;
					...
					SM.Corporate.ProductCatalog.Table.fnAddData([ ItemName, ItemDes, ItemPartNumber, ItemSKU,barcode, ItemQuantity,
						...
						if(data.keyExist == "0")
						{
							 var hideprice = "False";
								...
								TemHTML += '<td style="font-size: 13px;width: 100px;">'+$("#item_sku").val()+'</td>';
                                                                TemHTML += '<td style="font-size: 13px;width: 100px;">'+$("#brcd").val()+'</td>';
...
$(document).ready(function()
{
	...
	$("#catalog_import_upload_control").change(function()
	{
		...
		if ($.inArray(ext, [ 'csv' ]) == -1)
		{
			...
			showWarning('Only CSV Format Valid!');
			input.files[0] = "";
			return		
