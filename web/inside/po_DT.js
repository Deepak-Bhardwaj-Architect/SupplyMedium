...
function showPOPopupForm(aData)
{
    ...
    $('#po_tl').text("PO date: "+PODate);
    $('#typ').val(aData[34]);    
    $('#outside_supplier_address').val(aData[35]);
    $("#status").val(status);
...
function parsePORequest(poArr)
{
    ...
    for (var i = 0; i < poArr.length; i++)
    {
        ...
        if(isOutsideSupplier==="1")
                {
                   ...
                   var outsideSupplieraddress=po.outsideSupplieraddress.split(",")
        ...
        {
            //alert(toCompName);
            poDataTable.fnAddData([transImg, POId, PODate, toCompName, toRegnKey, toUserKey, toState, status, open,
                ...
                items, trans, inquires, transId, totListPrice, tax, totPrice, isInvoiceCreated,"sent",outsideSupplieraddressdetail]);
        }
        else
        {
            //alert('2');
            poDataTable.fnAddData([transImg, POId, PODate, fromCompName, fromRegnKey, fromUserKey, fromState + inquireChat, status, open,
                ...
                toCity, toAddress, toZipcode, isOutsideSupplier, outsideSupplierEmail, recurring, poRef,
                items, trans, inquires, transId, totListPrice, tax, totPrice, isInvoiceCreated,"rec",outsideSupplieraddressdetail]);
        }
        //alert('end');
    }
