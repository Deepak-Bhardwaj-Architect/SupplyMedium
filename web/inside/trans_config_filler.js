...
var quantityUnitsdetail = new Array();

var currencyList = new Array();
var currencyListdetail = new Array();
					...
					$.each(data, function(key, value) 
					{
						if( value != "success")
						{
							quantityUnits.push(key);
                                                        quantityUnitsdetail.push(value);
						}
...
function showQuantityUnits(  )
{
	...
	for( var i=0;i<quantityUnits.length;i++ )
	{
		...
		quantityUnitDiv = '<div class="quan_item" id="'+quantity+'_'+divId+'" title='+quantityUnitsdetail[i]+'>'+quantity+'</div>';
...
function showCurrencyList()
{
	...
	for( var i=0;i<currencyList.length;i++ )
	{
		...
		currencyUnitDiv = '<div class="currency_name" id="'+currency+'_'+divId+'" title='+currencyListdetail[i]+'>'+currency+'</div>';
